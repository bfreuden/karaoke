...
def create_karaoke(project_dir, force=False):
    # default values
    ...
    with open(f'{project_dir}/_data.json', mode='r') as fp:
        karaoke_project_data = json.load(fp)

    youtube_url = karaoke_project_data['youtube_url']
    genius_url = karaoke_project_data['genius_url']

    language = karaoke_project_data['language']  if 'language' in karaoke_project_data else 'en'

    speech_to_text_target = karaoke_project_data[
        'speech_to_text_target'] if 'speech_to_text_target' in karaoke_project_data else default_speech_to_text_target
...
if __name__ == '__main__':
    ...
    for project_name in project_names:
        try:
            ...
            force = True
            create_karaoke(project_dir, force)
